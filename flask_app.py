...
def get_walks(day, user_id):
    ...
    walks = [uwalk["duration"] for uwalk in user_walks]

    user_sunny_walks = list(db.walk.find({
        'user_id': user_id,
        'time_bucket': {
            '$gte': day,
            '$lt': day + timedelta(days=1)
    ...
    sunny_walks = [uwalk["duration"] for uwalk in user_sunny_walks]

    return {"date": "2019-07-16", "walked": sum(walks), "walked_in_the_sun": sum(sunny_walks)}
...
def get_user_walks_today():
    ...
    return jsonify(get_walks(today, user_id))
...
def get_user_walks_week():
    ...
    for day_i in range(8, 1, -1):
        day = datetime.combine(ddate.today() - timedelta(days=day_i), datetime.min.time())

        week_arr.append(get_walks(day, user_id))

    return jsonify(week_arr)


@application.route('/future')
def predict_future():
    """
    ...
    response = requests.get(f'https://api.darksky.net/forecast/aec3225ddd6f2dc01389770d446297d3/{lat},{lon}')
    results = response.json()
    ...
    for hour in results["hourly"]["data"]:
        ...
        if hour["cloudCover"] < 0.50 and get_light(lat, lon, hour.split()[0], hour.split()[1]):
            arr.append(time)
