...
class CalendarList extends Component {
  state = {
    ...
    data: [
      {
        from: "2019-11-17T12:00:00.000Z",
        to: "2019-11-17T13:00:00.000Z",
        type: "sunny",
        title: "Sunny Weather"
      },
      {
        from: "2019-11-19T12:00:00.000Z",
        to: "2019-11-19T12:00:00.000Z",
        type: "sunny",
        title: "Sunny Weather"
      },
      {
        from: "2019-11-20T12:00:00.000Z",
        to: "2019-11-20T12:00:00.000Z",
        type: "sunny",
        title: "Sunny Weather"
      },
      {
        from: "2019-11-20T13:00:00.000Z",
        to: "2019-11-20T13:00:00.000Z",
        type: "sunny",
        title: "Sunny Weather"
      },
      {
        from: "2019-11-20T14:00:00.000Z",
        to: "2019-11-20T14:00:00.000Z",
        type: "sunny",
        title: "Sunny Weather"
      },
      {
        from: "2019-11-20T15:00:00.000Z",
        to: "2019-11-20T15:00:00.000Z",
        type: "sunny",
        title: "Sunny Weather"
      }
    ],
  ...
  setCalendarEvent = async (specificCalendar) => {
    const event = await Calendar.createEventAsync(specificCalendar, {
      startDate: currentEvent["from"],
      endDate: currentEvent["to"],
  ...
  render() {
    const emptyList = (
      <View key={"-1"} style={styles.topText}>
        <Text>You don't have any calendar events yet.</Text>
      </View>
    );
    return (
      <View>
        ...
        >
          <View style={{ marginTop: 22 }}>
            <View>
              <Text>Please select your event:</Text>
              <Picker
                selectedValue={this.state.selectedCalendar}
                style={{ height: 50, width: 100 }}
                onValueChange={(itemValue, itemIndex) =>
                  this.setState({ calendarSelection: itemValue })
                }
              >
                {this.state.calendars.map(obj => (
                  <Picker.Item label={obj.title} value={obj.id} />
                ))}
              </Picker>
          ...
          renderItem={({ item }) => {
            if (item.type === "sunny") {
              return (
                <TouchableOpacity
                  onPress={event =>
                    this.addEvent(event, item.title, item.from, item.to)
                  }
                >
                  <View style={[styles.calendarItem, styles.sunny]}>
                    <Text style={styles.title}>{item.title}</Text>
                    <Text style={styles.dateobject}>
                      {moment(item.from).format("dddd, MMMM Do, hA")} -{" "}
                      {moment(item.to).format("dddd, MMMM Do, hA")}
                    </Text>
                  </View>
                </TouchableOpacity>
              );
              ...
              return (
                <View style={[styles.calendarItem, styles.normal]}>
                  <Text style={styles.title}>{item.title}</Text>
                  <Text style={styles.dateobject}>
                    {moment(item.from).format("dddd, MMMM Do, hA")}
                  </Text>
  ...
  getCalendarPermissions = async () => {
    ...
    if (status !== "denied") {
      if (status === "undetermined") {
        const { newStatus, newPermissions } = await Permissions.askAsync(
          Permissions.CALENDAR
        );
  ...
  checkEventoverlap = (weatherData, events) => {
    let newEvents = weatherData.filter(obj => {
      ...
      events.forEach(event => {
        if (
          weatherFrom > new Date(event["startDate"]) &&
